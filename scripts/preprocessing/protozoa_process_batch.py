...
print(f"[INFO] Processing batch {args.start}–{args.end} ({len(slice_files)} genomes)")
if not slice_files:
    print("[WARNING] No FASTA files found in slice.")
    sys.exit(0)
...
print(f"[INFO] Batch {args.start}-{args.end} complete. Report saved to {csv_path}")
time.sleep(1)