...
# Set your cutoff date (e.g., download genomes released BEFORE this date)
MIN_DATE = datetime.strptime("2023-03-01", "%Y-%m-%d")
...
# Extract relevant ftp paths (filtered by release date)
ftp_paths = []
with open(summary_file) as f:
    for line in f:
        if line.startswith("#"):
...
print(f"[INFO] Filtered to {len(ftp_paths)} genomes released before {MIN_DATE.date()}.")
...
# Download genomes
print(f"[INFO] Downloading genomes for '{CATEGORY}'...")
